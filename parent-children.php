		...
		$pcConfig = array(
			...
			'register_items' => array(   
				'previous_order' => array(   
					...
					'forced-where' => '',
					'display-fields' => array(2 => 'Item name', 3 => 'Cost', 4 => 'Image'),
					'display-field-names' => array(2 => 'item_name', 3 => 'cost', 4 => 'ref_img'),
					'sortable-fields' => array(0 => '`register_items`.`id`', 1 => '`register_items`.`previous_order`', 2 => '`register_items`.`item_name`', 3 => '`register_items`.`cost`', 4 => '`register_items`.`ref_img`'),
					'records-per-page' => 10,
					...
					'template' => 'children-register_items',
					'template-printable' => 'children-register_items-printable',
					'query' => "SELECT `register_items`.`id` as 'id', IF(    CHAR_LENGTH(`register1`.`id`), CONCAT_WS('',   `register1`.`id`), '') as 'previous_order', IF(    CHAR_LENGTH(`inventory1`.`item`), CONCAT_WS('',   `inventory1`.`item`), '') as 'item_name', IF(    CHAR_LENGTH(`inventory1`.`price_high`), CONCAT_WS('',   `inventory1`.`price_high`), '') as 'cost', IF(    CHAR_LENGTH(`inventory1`.`inventory_photo`), CONCAT_WS('',   `inventory1`.`inventory_photo`), '') as 'ref_img' FROM `register_items` LEFT JOIN `register` as register1 ON `register1`.`id`=`register_items`.`previous_order` LEFT JOIN `inventory` as inventory1 ON `inventory1`.`id`=`register_items`.`item_name` "
				)
			)
		);
